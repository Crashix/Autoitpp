// AutoIt++X3.cpp : Defines the exported functions for the DLL application.
//
#include "stdafx.h"
#include "AutoIt++X3.h"
#include <sstream>
...
#define S2WS(a) s2ws(a).c_str()
#define WS2S(a) wstrtostr(a)
#define S2N(a,b) strtonumber<b>(a)
...
namespace AU3{
	...
	long AutoItSetOption(std::string szOption, long nValue){
		return AU3_AutoItSetOption(S2WS(szOption), nValue);
	}

	void BlockInput(long nFlag){
		AU3_BlockInput(nFlag);
	}

	long CDTray(std::string szDrive, std::string szAction){
		return AU3_CDTray(S2WS(szDrive), S2WS(szAction));
	}
	std::string ClipGet(){
		WCHAR szClip[5000];
		AU3_ClipGet(szClip,5000);
		return WS2S(szClip);
	}
	void ClipPut(std::string szClip){
		AU3_ClipPut(S2WS(szClip));
	}
	long ControlClick(std::string szTitle, std::string szText, std::string szControl, std::string szButton, long nNumClicks, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nX, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nY){
		return AU3_ControlClick(S2WS(szTitle), S2WS(szText),S2WS(szControl),S2WS(szButton),nNumClicks,nX,nY);
	}
	std::string ControlCommand(std::string szTitle, std::string szText, std::string szControl, std::string szCommand, std::string szExtra){

		WCHAR szResult[50];
		AU3_ControlCommand(S2WS(szTitle),S2WS(szText),S2WS(szControl),S2WS(szCommand),S2WS(szExtra),szResult,50);
		return WS2S(szResult);
	}
	std::string  ControlListView(std::string szTitle, std::string szText, std::string szControl, std::string szCommand, std::string szExtra1, std::string szExtra2){
		WCHAR szResult[50];
		AU3_ControlListView(S2WS(szTitle),S2WS(szText),S2WS(szControl),S2WS(szCommand),S2WS(szExtra1),S2WS(szExtra2),szResult,50);
		return WS2S(szResult);
	}
	long ControlDisable(std::string szTitle, std::string szText, std::string szControl){
		return AU3_ControlDisable(S2WS(szTitle), S2WS(szText), S2WS(szControl));
	}
	long ControlEnable(std::string szTitle, std::string szText, std::string szControl){
		return AU3_ControlEnable(S2WS(szTitle),S2WS(szText),S2WS(szControl));
	}
	long ControlFocus(std::string szTitle, std::string szText, std::string szControl){
		return AU3_ControlFocus(S2WS(szTitle),S2WS(szText),S2WS(szControl));
	}
	long ControlGetHandle(std::string szTitle, std::string szText, std::string szControl){
		WCHAR szResult[200];
		AU3_ControlGetHandle(S2WS(szTitle),S2WS(szText),S2WS(szControl),szResult,200);
		std::string resString = WS2S(szResult);
		return S2N(resString,long);
	}
	std::vector<long> ControlGetPos(std::string szTitle, std::string szText, std::string szControl){
		std::vector<long> szResultArray(4,-1);
		LPCWSTR szTitleWSTR = S2WS(szTitle);
		LPCWSTR szTextWSTR = S2WS(szTitle);
		LPCWSTR szControlWSTR = S2WS(szControl);
		szResultArray[0] = AU3_ControlGetPosX(szTitleWSTR,szTextWSTR,szControlWSTR);
		szResultArray[1] = AU3_ControlGetPosY(szTitleWSTR,szTextWSTR,szControlWSTR);
		szResultArray[2] = AU3_ControlGetPosWidth(szTitleWSTR,szTextWSTR,szControlWSTR);
		szResultArray[3] = AU3_ControlGetPosHeight(szTitleWSTR,szTextWSTR,szControlWSTR);
		return szResultArray;
	}
	std::string ControlGetText(std::string szTitle, std::string szText, std::string szControl){
		WCHAR szControlText[5000];
		AU3_ControlGetText(S2WS(szTitle),S2WS(szText),S2WS(szControl),szControlText,5000);
		return WS2S(szControlText);
	}
	long ControlHide(std::string szTitle, std::string szText, std::string szControl){
		return AU3_ControlHide(S2WS(szTitle),S2WS(szText),S2WS(szControl));
	}
	long ControlMove(std::string szTitle, std::string szText, std::string szControl, long nX, long nY, /*[in,defaultvalue(-1)]*/long nWidth, /*[in,defaultvalue(-1)]*/long nHeight){
		return AU3_ControlMove(S2WS(szTitle),S2WS(szText),S2WS(szControl),nX,nY,nWidth,nHeight);
	}
	long ControlSend(std::string szTitle, std::string szText, std::string szControl, std::string szSendText, /*[in,defaultvalue(0)]*/long nMode){
		return AU3_ControlSend(S2WS(szTitle),S2WS(szText),S2WS(szControl),S2WS(szSendText),nMode);
	}
	long ControlSetText(std::string szTitle, std::string szText, std::string szControl, std::string szControlText){
		return AU3_ControlSetText(S2WS(szTitle),S2WS(szText),S2WS(szControl),S2WS(szControlText));
	}
	long ControlShow(std::string szTitle, std::string szText, std::string szControl){
		return AU3_ControlShow(S2WS(szTitle),S2WS(szText),S2WS(szControl));
	}
	std::string ControlTreeView(std::string szTitle, std::string szText, std::string szControl, std::string szCommand, std::string szExtra1, std::string szExtra2){
		WCHAR szResult[5000];
		//AU3_ControlTreeView(S2WS(szTitle),S2WS(szText),S2WS(szControl),S2WS(szCommand),S2WS(szExtra1),S2WS(szExtra2),szResult,5000);
		return WS2S(szResult);
	}



	std::string DriveMapAdd(std::string szDevice, std::string szShare, long nFlags, /*[in,defaultvalue("")]*/std::string szUser, /*[in,defaultvalue("")]*/std::string szPwd){
		WCHAR szResult[5000];
		AU3_DriveMapAdd(S2WS(szDevice),S2WS(szShare),nFlags,S2WS(szUser),S2WS(szPwd),szResult,5000);
		return WS2S(szResult);
	}
	long DriveMapDel(std::string szDevice){
		return AU3_DriveMapDel(S2WS(szDevice));
	}
	std::string DriveMapGet(std::string szDevice){
		WCHAR szResult[5000];
		AU3_DriveMapGet(S2WS(szDevice),szResult,5000);
		return WS2S(szResult);
	}



	long IniDelete(std::string szFilename, std::string szSection, std::string szKey){
		return AU3_IniDelete(S2WS(szFilename),S2WS(szSection),S2WS(szKey));
	}
	std::string IniRead(std::string szFilename, std::string szSection, std::string szKey, std::string szDefault){
		WCHAR szValue[5000];
		AU3_IniRead(S2WS(szFilename),S2WS(szSection),S2WS(szKey),S2WS(szDefault),szValue,5000);
		return WS2S(szValue);
	}
	long IniWrite(std::string szFilename, std::string szSection, std::string szKey, std::string szValue){
		return AU3_IniWrite(S2WS(szFilename),S2WS(szSection),S2WS(szKey),S2WS(szValue));
	}
	...
	long MouseClick(/*[in,defaultvalue("LEFT")]*/std::string szButton, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nX, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nY, /*[in,defaultvalue(1)]*/long nClicks, /*[in,defaultvalue(-1)]*/long nSpeed){
		return AU3_MouseClick(S2WS(szButton),nX,nY,nClicks,nSpeed);
	}
	long MouseClickDrag(std::string szButton, long nX1, long nY1, long nX2, long nY2, /*[in,defaultvalue(-1)]*/long nSpeed){
		return AU3_MouseClickDrag(S2WS(szButton),nX1,nY1,nX2,nY2,nSpeed);
	}
	void MouseDown(/*[in,defaultvalue("LEFT")]*/std::string szButton){
		AU3_MouseDown(S2WS(szButton));
	}
	long MouseGetCursor(void){
		return AU3_MouseGetCursor();
	}
	long MouseGetPosX(void){return AU3_MouseGetPosX();}
	long MouseGetPosY(void){return AU3_MouseGetPosY();}
	long MouseMove(long nX, long nY, /*[in,defaultvalue(-1)]*/long nSpeed){
		return AU3_MouseMove(nX,nY,nSpeed);
	}
	void MouseUp(/*[in,defaultvalue("LEFT")]*/std::string szButton){
		AU3_MouseUp(S2WS(szButton));
	}
	void MouseWheel(std::string szDirection, long nClicks){
		AU3_MouseWheel(S2WS(szDirection),nClicks);
	}



	long Opt(std::string szOption, long nValue){
		return AU3_Opt(S2WS(szOption),nValue);
	}



	unsigned long PixelChecksum(long nLeft, long nTop, long nRight, long nBottom, /*[in,defaultvalue(1)]*/long nStep){
		return AU3_PixelChecksum(nLeft,nTop,nRight,nBottom,nStep);
	}
	long PixelGetColor(long nX, long nY){
		return AU3_PixelGetColor(nX,nY);
	}
	LPPOINT PixelSearch(long nLeft, long nTop, long nRight, long nBottom, long nCol, /*default 0*/long nVar, /*default 1*/long nStep){
		LPPOINT pPointResult = new POINT;
		AU3_PixelSearch(nLeft,nTop,nRight,nBottom,nCol,nVar,nStep,pPointResult);
		return pPointResult;
	}
	long ProcessClose(std::string szProcess){
		return AU3_ProcessClose(S2WS(szProcess));
	}
	long ProcessExists(std::string szProcess){
		return AU3_ProcessExists(S2WS(szProcess));
	}
	long ProcessSetPriority(std::string szProcess, long nPriority){
		return AU3_ProcessSetPriority(S2WS(szProcess),nPriority);
	}
	long ProcessWait(std::string szProcess, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_ProcessWait(S2WS(szProcess),nTimeout);
	}
	long ProcessWaitClose(std::string szProcess, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_ProcessWaitClose(S2WS(szProcess),nTimeout);
	}
	long RegDeleteKey(std::string szKeyname){
		return AU3_RegDeleteKey(S2WS(szKeyname));
	}
	long RegDeleteVal(std::string szKeyname, std::string szValuename){
		return AU3_RegDeleteVal(S2WS(szKeyname),S2WS(szValuename));
	}
	std::string RegEnumKey(std::string szKeyname, long nInstance){
		WCHAR szResult[5000];
		AU3_RegEnumKey(S2WS(szKeyname),nInstance,szResult,5000);
		return WS2S(szResult);
	}
	std::string RegEnumVal(std::string szKeyname, long nInstance){
		WCHAR szResult[5000];
		AU3_RegEnumVal(S2WS(szKeyname),nInstance,szResult,5000);
		return WS2S(szResult);
	}
	std::string RegRead(std::string szKeyname, std::string szValuename){
		WCHAR szResult[5000];
		AU3_RegRead(S2WS(szKeyname),S2WS(szValuename),szResult,5000);
		return WS2S(szResult);
	}
	std::string RegReadVal(std::string path, std::string val,int valnumber){
		std::string hu = "";
		for (int i = 1; i < valnumber + 1; i++)
		{
			if(AU3::RegEnumVal(path,i) == val){
				hu = AU3::RegEnumVal(path,i);
			}
		}
		return AU3::RegRead(path,hu);
	}
	long RegWrite(std::string szKeyname, std::string szValuename, std::string szType, std::string szValue){
		return AU3_RegWrite(S2WS(szKeyname),S2WS(szValuename),S2WS(szType),S2WS(szValue));
	}
	long Run(std::string szRun, /*[in,defaultvalue("")]*/std::string szDir, /*[in,defaultvalue(1)]*/Run_opt_flag nShowFlags){
		return AU3_Run(S2WS(szRun),S2WS(szDir),nShowFlags);
	}
	long RunAsSet(std::string szUser, std::string szDomain, std::string szPassword, int nOptions){
		return AU3_RunAsSet(S2WS(szUser),S2WS(szDomain),S2WS(szPassword),nOptions);
	}
	long RunWait(std::string szRun, /*[in,defaultvalue("")]*/std::string szDir, /*[in,defaultvalue(1)]*/Run_opt_flag nShowFlags){
		return AU3_RunWait(S2WS(szRun),S2WS(szDir),nShowFlags);
	}



	void Send(std::string szSendText,long nMode){
		AU3_Send(S2WS(szSendText), nMode);
	}
	void SendA(std::string szSendText, /*[in,defaultvalue("")]*/long nMode){
		AU3_Send(S2WS(szSendText), nMode);
	}
	long Shutdown(long nFlags){
		return AU3_Shutdown(nFlags);
	}
	void Sleep(long nMilliseconds){
		AU3_Sleep(nMilliseconds);
	}
	std::string StatusbarGetText(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, /*[in,defaultvalue(1)]*/long nPart){
		WCHAR LPCRes[300];
		AU3_StatusbarGetText(S2WS(szTitle),S2WS(szText),nPart,LPCRes,300);
		;
		return WS2S(LPCRes);
	}
	...
	void ToolTip(std::string szTip, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nX, /*[in,defaultvalue(AU3_INTDEFAULT)]*/long nY){
		AU3_ToolTip(S2WS(szTip),nX,nY);
	}



	void WinActivate(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		AU3_WinActivate(S2WS(szTitle),S2WS(szText));
	}
	long WinActive(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		return AU3_WinActive(S2WS(szTitle),S2WS(szText));
	}
	long WinClose(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		return AU3_WinClose(S2WS(szTitle),S2WS(szText));
	}
	long WinExists(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		return AU3_WinExists(S2WS(szTitle),S2WS(szText));
	}
	LPPOINT WinGetCaretPos(void){
		LPPOINT pos = new POINT;
		pos -> x = AU3_WinGetCaretPosX();
		pos -> y = AU3_WinGetCaretPosY();
		return pos;
	}
	std::string WinGetClassList(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		WCHAR szRetText[5000];
		AU3_WinGetClassList(S2WS(szTitle),S2WS(szText),szRetText,5000);
		return WS2S(szRetText);
	}
	std::vector<long> WinGetClientSize(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		std::vector<long> ClientSize(2);
		LPCWSTR szTitleWSTR = S2WS(szTitle);
		LPCWSTR szTextWSTR = S2WS(szText);
		ClientSize[0] = AU3_WinGetClientSizeHeight(szTitleWSTR,szTextWSTR);
		ClientSize[1] = AU3_WinGetClientSizeWidth(szTitleWSTR,szTextWSTR);
		return ClientSize;
	}
	long WinGetHandle(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		WCHAR szRetText[5000];
		AU3_WinGetHandle(S2WS(szTitle),S2WS(szText),szRetText,5000);
		return S2N(WS2S(szRetText),long);
	}
	std::vector<long> WinGetPos(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		std::vector<long> ClientPos(4,-1);
		LPCWSTR szTitleWSTR = S2WS(szTitle);
		LPCWSTR szTextWSTR = S2WS(szText);
		ClientPos[0] = AU3_WinGetPosX(szTitleWSTR,szTextWSTR);
		ClientPos[1] = AU3_WinGetPosY(szTitleWSTR,szTextWSTR);
		ClientPos[2] = AU3_WinGetPosWidth(szTitleWSTR,szTextWSTR);
		ClientPos[3] = AU3_WinGetPosHeight(szTitleWSTR,szTextWSTR);
		return ClientPos;
	}
	long WinGetProcess(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		WCHAR szRetText[5000];
		AU3_WinGetProcess(S2WS(szTitle),S2WS(szText),szRetText,5000);
		return S2N(WS2S(szRetText),long);
	}
	long WinGetState(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		return AU3_WinGetState(S2WS(szTitle),S2WS(szText));
	}
	std::string WinGetText(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		WCHAR szRetText[5000];
		AU3_WinGetText(S2WS(szTitle),S2WS(szText),szRetText,5000);
		return WS2S(szRetText);
	}
	std::string WinGetTitle(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		WCHAR szRetText[5000];
		AU3_WinGetTitle(S2WS(szTitle),S2WS(szText),szRetText,5000);
		return WS2S(szRetText);
	}
	long WinKill(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText){
		return AU3_WinKill(S2WS(szTitle),S2WS(szText));
	}
	long WinMenuSelectItem(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, std::string szItem1,
																							 std::string szItem2,
																							 std::string szItem3,
																							 std::string szItem4,
																							 std::string szItem5,
		...
		return AU3_WinMenuSelectItem(S2WS(szTitle),S2WS(szText),S2WS(szItem1),
																S2WS(szItem2),
																S2WS(szItem3),
																S2WS(szItem4),
																S2WS(szItem5),
	...
	void WinMinimizeAll(){AU3_WinMinimizeAll();}
	void WinMinimizeAllUndo(){AU3_WinMinimizeAllUndo();}
	long WinMove(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, long nX, long nY, /*[in,defaultvalue(-1)]*/long nWidth, /*[in,defaultvalue(-1)]*/long nHeight){
		return AU3_WinMove(S2WS(szTitle),S2WS(szText),nX,nY,nWidth,nHeight);
	}
	long WinSetOnTop(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, long nFlag){
		return AU3_WinSetOnTop(S2WS(szTitle),S2WS(szText),nFlag);
	}
	long WinSetState(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, long nFlags){
		return AU3_WinSetState(S2WS(szTitle),S2WS(szText),nFlags);
	}
	long WinSetTitle(std::string szTitle,/*[in,defaultvalue("")]*/ std::string szText, std::string szNewTitle){
		return AU3_WinSetTitle(S2WS(szTitle),S2WS(szText),S2WS(szNewTitle));
	}
	long WinSetTrans(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, long nTrans){
		return AU3_WinSetTrans(S2WS(szTitle),S2WS(szText),nTrans);
	}
	long WinWait(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_WinWait(S2WS(szTitle),S2WS(szText),nTimeout);
	}
	long WinWaitActive(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_WinWaitActive(S2WS(szTitle),S2WS(szText),nTimeout);
	}
	long WinWaitClose(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_WinWaitClose(S2WS(szTitle),S2WS(szText),nTimeout);
	}
	long WinWaitNotActive(std::string szTitle, /*[in,defaultvalue("")]*/std::string szText, /*[in,defaultvalue(0)]*/long nTimeout){
		return AU3_WinWaitNotActive(S2WS(szTitle),S2WS(szText),nTimeout);
	}
	...
	#define ToWideCharSize(source) MultiByteToWideChar(CP_ACP, 0, source, -1, NULL, 0)
	#define ToWideChar(source, dest, dest_size_in_wchars) MultiByteToWideChar(CP_ACP, 0, source, -1, dest, dest_size_in_wchars) //call ToWideCharSize firstly for the size
	...
	// ratio is retained.
	{
		...
		int wchars_num = ToWideCharSize(aFilespec);
		wchar_t* wstr = new wchar_t[wchars_num];
		ToWideChar(aFilespec,wstr,wchars_num);
		...
		if (ExtractIcon_was_used)
		{
			...
			hbitmap = (HBITMAP)ExtractIcon(g_hInstance, wstr, aIconNumber > 0 ? aIconNumber - 1 : 0);
		...
		if (aUseGDIPlusIfAvailable && !(hinstGDI = LoadLibrary(L"gdiplus"))) // Relies on short-circuit boolean order for performance.
		...
		if (!hbitmap && aImageType > -1 && !aUseGDIPlusIfAvailable)
		{
			...
			if (hbitmap = (HBITMAP)LoadImage(NULL, wstr, aImageType, desired_width, desired_height
			...
			else if (GetFileAttributes(wstr) == 0xFFFFFFFF) // For simplicity, we don't check if it's a directory vs. file, since that should be too rare.
				return NULL;
			...
			else if (aIconNumber > 0)
			{
				...
				hbitmap = (HBITMAP)ExtractIcon(g_hInstance, wstr, aIconNumber - 1);
				if (hbitmap < (HBITMAP)2) // i.e. it's NULL or 1. Return value of 1 means "incorrect file type".
					return NULL;
		...
		if (!hbitmap) // Above hasn't loaded the image yet, so use the fall-back methods.
		{
			...
			if (aUseGDIPlusIfAvailable || !file_ext || (_stricmp(file_ext, "jpg")
				...
				if (!hinstGDI) // We don't yet have a handle from an earlier call to LoadLibary().
					hinstGDI = LoadLibrary(L"gdiplus");
			...
			else // Using old picture loading method.
			{
				...
				HANDLE hfile = CreateFile(wstr, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		if (keep_aspect_ratio)
		{
			...
			if (aImageType != IMAGE_BITMAP)
			{
				...
				if (!ExtractIcon_was_used)
				{
					...
					return (HBITMAP)LoadImage(NULL, wstr, aImageType, aWidth, aHeight, LR_LOADFROMFILE);
				}
			}
		}
	...
	char* WINAPI _ImageSearch(int aLeft, int aTop, int aRight, int aBottom, char *aImageFile)
	...
	bool _ImageSearchArea(char *aImageFile, int aLeft, int aTop, int aRight, int aBottom, std::vector<int> &res) // res = {locx,locy,image_width,image_height}
	{

		std::vector<std::string> resString;
		std::string answer = _ImageSearch(aLeft, aTop, aRight, aBottom, aImageFile);

		if (answer == "0") {return FALSE;}

		resString = split(answer,'|');

		for (unsigned int i = 1; i < resString.size(); i++)
		{
			res.push_back(atoi(resString[i].c_str()));
		}

		return TRUE;
	}

	bool ImageSearchArea(std::string aImageFile ,int aLeft, int aTop, int aRight, int aBottom,int &x, int &y, bool resultPosition)
	{

		std::vector<int> result;
		char *cstr = new char[aImageFile.length() + 1];
		strcpy_s(cstr, aImageFile.length() + 1,aImageFile.c_str());
		if(_ImageSearchArea(cstr, aLeft, aTop, aRight, aBottom,result))
		{
			x = result[0];
			y = result[1];
			if(resultPosition){
		...
		else{delete [] cstr;return FALSE;}

	}


	bool ImageSearch(std::string aImageFile ,int &x, int &y, bool resultPosition)
	{

		if(ImageSearchArea(aImageFile,0,0,DESKTOP_WIDTH,DESKTOP_HEIGHT,x,y,resultPosition))
		{
			return TRUE;
		}
		else{return FALSE;}
	...
	DWORD GetBaseAdress(std::string baseName,/*[out]*/ HANDLE* pHandle){
		...
		for (unsigned int i = 0; i < cProcesses; i++)
		{
			...
			HANDLE hProcess = OpenProcess( PROCESS_QUERY_INFORMATION | PROCESS_VM_READ | PROCESS_VM_OPERATION | PROCESS_VM_WRITE, 
								   FALSE, 
								   aProcesses[i] );
			...
			if( EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded))
			{
				...
				for ( unsigned int j = 0; j < (cbNeeded / sizeof(HMODULE)); j++ )
				{
					...
					GetModuleBaseName( hProcess,hMods[j], szName, MAX_PATH );

					if(WS2S(szName) == baseName){
						if(pHandle)*pHandle = hProcess;
						return (DWORD)hMods[j];
	...
	DWORD GetBaseAdress(std::string processName,std::string windowName,/*[out]*/ HANDLE* pHandle){

		DWORD aProcesses; //This will store our Process ID, used to read/write into the memory
		HWND hwnd; //Finally a handle to our window

		hwnd = FindWindow(NULL,S2WS(windowName));

		if(!hwnd)
			return NULL;

		GetWindowThreadProcessId(hwnd,&aProcesses);
